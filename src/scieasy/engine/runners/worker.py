...
ADR-027 D11 + ADR-031: per-item reconstruction delegates to
:func:`scieasy.core.types.serialization._reconstruct_one` which returns
typed :class:`~scieasy.core.types.base.DataObject` instances (e.g.
:class:`~scieasy.core.types.array.Array`).  Lazy loading is preserved at
the method level: returned instances have ``storage_ref`` set but do not
read payload data until ``to_memory()`` / ``sel()`` / ``iter_over()`` is
called.  Serialisation delegates symmetrically to
...
def reconstruct_inputs(payload: dict[str, Any]) -> dict[str, Any]:
    ...
    ADR-027 D11 + ADR-031: returns typed :class:`DataObject` instances
    (e.g. :class:`~scieasy.core.types.array.Array` or a plugin subclass
    like ``FluorImage``).  Lazy loading is preserved at the method level:
    returned instances have ``storage_ref`` set but do not read payload
    data until ``to_memory()`` / ``sel()`` / ``iter_over()`` is called.
